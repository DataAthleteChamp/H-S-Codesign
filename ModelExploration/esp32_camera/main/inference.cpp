...
#define TENSOR_ARENA_SIZE (5 * 1024 * 1024)
...
static constexpr float CROP_FRACTION = 0.75f;
...
void inference_preprocess(const uint8_t *rgb565_frame)
{
    ...
    const int base_crop_side = FRAME_W < FRAME_H ? FRAME_W : FRAME_H;
    const int crop_side = static_cast<int>(base_crop_side * CROP_FRACTION);
